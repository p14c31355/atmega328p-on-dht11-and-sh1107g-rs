    ...
    S: core::fmt::Write,
{
    ...
    fn backtrack<I2C, S>(
        ...
        addr: u8,
        prefix: u8,
        order: &mut heapless::Vec<usize, 17>,
        used: &mut [bool; 17],
    ) -> bool
    where
        I2C: embedded_hal::blocking::i2c::Write + embedded_hal::blocking::i2c::Read,
        ...
        S: core::fmt::Write,
    {
        if order.len() == cmds.len() {
            return check_vram(i2c, addr); // VRAMチェック
        }

        for i in 0..cmds.len() {
            if used[i] { continue; }
            if !cmds[i].deps.iter().all(|&d| used[d]) { continue; }
            ...
            if backtrack(cmds, i2c, serial, addr, prefix, order, used) {
                return true;
            }
    ...
    if backtrack(cmds, i2c, serial, addr, prefix, &mut order, &mut used) {
        Ok(order)
    } else {
