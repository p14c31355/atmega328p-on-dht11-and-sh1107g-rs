...
fn main() -> ! {
    ...
    if run_auto_vram_backtrack_all(&EXPLORER_CMDS, &mut i2c, &mut serial, addr, prefix, &mut all_sequences) {
        ...
        for (idx, seq) in all_sequences.iter().enumerate() {
            ...
            for &node in seq.iter() {
                ...
                if i2c.write(addr, &buf[..1+cmd.bytes.len()]).is_err() {
                    let _ = writeln!(serial, "[Fail] Node {} write failed during sequence #{}", node, idx+1);
                    break;
                }
                // 読み出し検証
                let mut read_buf = [0u8; BUF_CAP];
                if i2c.read(addr, &mut read_buf[..cmd.bytes.len()]).is_err() {
                    let _ = writeln!(serial, "[Fail] Node {} read failed during sequence #{}", node, idx+1);
                    break;
                }
                if &read_buf[..cmd.bytes.len()] != cmd.bytes {
                    let _ = writeln!(serial, "[Fail] Node {} verification mismatch during sequence #{} expected={:02X?} read={:02X?}",
...
fn run_auto_vram_backtrack_all<I2C, S>(
    cmds: &[CmdNode],
    i2c: &mut I2C,
    serial: &mut S,
    addr: u8,
    ...
    S: core::fmt::Write,
{
        ...
        S: core::fmt::Write,
    {
        if sequence.len() == cmds.len() {
            let _ = writeln!(serial, "[Info] Found valid sequence: {:?}", sequence);
        ...
        for &node in candidates.iter() {
            ...
            let mut read_buf = [0u8; BUF_CAP];
            if i2c.read(addr, &mut read_buf[..cmd.bytes.len()]).is_err() {
                let _ = writeln!(serial, "[Fail] Node {} I2C read failed bytes={:02X?}", node, cmd.bytes);
                continue;
            }
            if &read_buf[..cmd.bytes.len()] != cmd.bytes {
                let _ = writeln!(serial, "[Fail] Node {} verification mismatch expected={:02X?} read={:02X?}",
            ...
            if backtrack(cmds, i2c, serial, addr, prefix, in_degree, sequence, visited, all_sequences) {
                success = true;
            }
    ...
    backtrack(cmds, i2c, serial, addr, prefix, &mut in_degree, &mut sequence, &mut visited, all_sequences)
}
