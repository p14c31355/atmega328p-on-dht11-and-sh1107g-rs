...
fn main() -> ! {
    ...
    // -------------------------
    // OLED 初期化（SRAM フル使用回避）
    // -------------------------
    // clear_on_init を false にして初期化
    let mut oled = Sh1107gBuilder::new(i2c)
        .clear_on_init(false)
        .build();

    // init() は 64バイトごとに送信するので安全
    if oled.init().is_ok() {
    ...
    let rect = Rectangle::new(
        Point::new((DISPLAY_WIDTH/2 - 10) as i32, (DISPLAY_HEIGHT/2 - 10) as i32),
        Size::new(20, 20)
    );
    let _ = oled.draw_iter(rect.points().map(|p| Pixel(p, BinaryColor::On)));

    // flush はページ単位で 64 バイトごと送信
    oled.flush().ok();
